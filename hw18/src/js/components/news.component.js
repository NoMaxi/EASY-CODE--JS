...
export class NewsComponent {
    /** Create authentication, news, routing service and define the authentication user token */
    constructor() {
     ...
     */
    async beforeRender() {
        // navigate to login form if no user token provided
        if (!this._authService.token) {
            console.log('No user token provided. Please login to your account.');
            this._routingService.navigate('/login');
        }

        // get an array of the latest news added by users
        this._usersNews = await this._newsService.getNews(this._authUserToken);
